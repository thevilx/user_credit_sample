...
import { EventType, PaymentRequest, PaymentStatus, TransactionType } from '@prisma/client';
...
export class PaymentProcessor {
    ...
    @EventPattern(RabbitMQEvents.PROCESS_PAYMENT)
    async handlePayment(
        ...
        const pr = await this.resolvePaymentRequest(data, context);
        if (!pr) return;

        if (this.isTerminal(pr, context)) return;

        await this.markProcessing(pr);

        const lock = await this.acquireUserLock(pr);
        try {
            this.simulateTransientFailure();
            if (await this.checkBusinessRules(pr, context)) return;

            const result = await this.executePayment(pr);
            if (!result.success) {
                await this.handleBusinessFailure(pr, result.reason, context);
                return;
            }
            ...
            await this.handleTechnicalFailure(pr, err, context);
        } finally {
    ...
    private async resolvePaymentRequest(
        ...
        if (!pr) {
            this.logger.warn(`PaymentRequest not found: ${data.paymentRequestId}`);
            this.rabbitMQService.ack(context);
            return null;
        }
    ...
    private isTerminal(pr: PaymentRequest, context: RmqContext): boolean {
        ...
        if (terminalStates.includes(pr.status)) {
            this.logger.warn(`Already in terminal state: ${pr.status}`);
            this.rabbitMQService.ack(context);
            return true;
        }
    ...
    private async markProcessing(pr: PaymentRequest) {
    ...
    private async acquireUserLock(pr: PaymentRequest) {
        const lock = await this.redis.acquireLock(`user:${pr.userId}`, 8000);
        this.logger.log(`Lock acquired for user: ${pr.userId}`);
        return lock;
    }

    private simulateTransientFailure() {
        if (Math.random() < 0.05) {
    ...
    private async checkBusinessRules(
        ...
        if (pr.reference.includes('FAIL')) {
            this.logger.warn(`Reference forced failure: ${pr.reference}`);
            await this.failPayment(pr.id, 'Reference pattern forced failure');
            this.rabbitMQService.ack(context);
            return true;
        }

        if (this.amountExceedsRiskThreshold(pr.amount)) {
            this.logger.warn(`Amount risk check failed: ${pr.amount}`);
    ...
    private amountExceedsRiskThreshold(amount: any): boolean {
    ...
    private async executePayment(pr: PaymentRequest): Promise<{ success: boolean; reason?: string }> {
    ...
    private async handleBusinessFailure(
        pr: PaymentRequest,
        reason: string,
        context: RmqContext,
    ) {
        this.logger.warn(`Transaction failed: ${reason}`);
        await this.failPayment(pr.id, reason);
        this.rabbitMQService.ack(context);
    }
    ...
    private async handleTechnicalFailure(
        ...
        if (pr.retryCount < MAX_RETRIES) {
            await this.retryPayment(pr, err, context);
        } else {
            await this.exhaustRetries(pr, err, context);
        }
    }

    private async retryPayment(pr: PaymentRequest, err: Error, context: RmqContext) {
        ...
        this.rabbitMQService.nack(context, false);
    }

    private async exhaustRetries(pr: PaymentRequest, err: Error, context: RmqContext) {
